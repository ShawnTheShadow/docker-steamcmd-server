...
if [ "${CARBON_MOD}" == "true" ]; then
  ...
  CUR_V="$(find ${SERVER_DIR} -maxdepth 1 -name "CarbonMod-*.tar.gz" | cut -d '-' -f2 | grep -Eo '([0-9]+(\.[0-9]+)+)')"
  TAG_V="$(wget -qO- https://api.github.com/repos/CarbonCommunity/Carbon/releases/latest | grep tag_name | cut -d '"' -f4)"
  LAT_V="$(wget -qO- https://api.github.com/repos/CarbonCommunity/Carbon/releases/latest | grep -m 1 '"name"' | cut -d '"' -f4 | grep -Eo '([0-9]+(\.[0-9]+)+)')"
  ...
  if [ -z "${CUR_V%.}" ]; then
    ...
    cd ${SERVER_DIR}
    if wget -q -nc --show-progress --progress=bar:force:noscroll -O ${SERVER_DIR}/CarbonMod-${LAT_V}.tar.gz "https://github.com/CarbonCommunity/Carbon/releases/download/${TAG_V}/Carbon.Linux.Release.tar.gz" ; then
